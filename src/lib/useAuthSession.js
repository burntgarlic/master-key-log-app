...
import { syncJournalOnLogin, clearJournalSyncUser } from './journalSync.js'
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (data.session?.user) {
        ...
        syncJournalOnLogin(data.session.user.id)
      }
    })
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange((_event, newSession) => {
      ...
      if (newSession?.user) {
        ...
        syncJournalOnLogin(newSession.user.id)
      } else {
        clearSyncUser()
        clearJournalSyncUser()
      }
    })
